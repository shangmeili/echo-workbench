...
try {
  ...
  assert.match(rangedImportRows[0], /00:05.000 - 00:08.000/);
  ...
  assert.match(rangedImportRows[1], /00:08.000 - 00:12.000/);
  assert.match(rangedImportRows[1], /Speaker/, "proofreading rows should keep speaker labels visible when imported from text");
  assert.match(rangedImportRows[1], /Explicit range line/);
  assert.match(rangedImportRows[2], /00:12.500 - 00:14.000/);
  assert.match(rangedImportRows[2], /旁白/, "localized speaker labels should remain visible in the proofreading list");
  assert.match(rangedImportRows[2], /小数时间范围/);
  ...
  const manualImportRecent = await page.evaluate(() => {
    ...
    return data.find((item) => /导入文本 · 3 条/.test(item.meta || "")) || null;
  });
  ...
  assert.match(await page.evaluate(() => navigator.clipboard.readText()), /\[00:05\.000 - 00:08\.000\][\s\S]*校对稿：带时间范围的开场/, "current segment copy should include timing and current text");
  ...
  await page.waitForFunction(() => document.querySelector(".review-list-row")?.innerText.includes("00:05.700 - 00:08.700"));
  assert.match(await page.locator(".review-list-row").first().innerText(), /00:05.700 - 00:08.700/, "time sync should shift all row timecodes later by the configured offset");
  await page.getByRole("button", { name: "撤销", exact: true }).click();
  await page.waitForFunction(() => document.querySelector(".review-list-row")?.innerText.includes("00:05.000 - 00:08.000"));
  await page.getByRole("button", { name: "整体提前", exact: true }).click();
  await page.waitForFunction(() => document.querySelector(".review-list-row")?.innerText.includes("00:04.300 - 00:07.300"));
  assert.match(await page.locator(".review-list-row").first().innerText(), /00:04.300 - 00:07.300/, "time sync should shift all row timecodes earlier by the configured offset");
  await page.getByRole("button", { name: "撤销", exact: true }).click();
  await page.waitForFunction(() => document.querySelector(".review-list-row")?.innerText.includes("00:05.000 - 00:08.000"));
  await openMediaAssociation(page);
  ...
  assert.match(await repairStatus.innerText(), /需修复 1 条/, "hard export blockers should be visible before the user tries to export");
  assert.equal(await repairStatus.isEnabled(), true, "hard export blocker status should be clickable");
  const blockedExportButton = page.locator(".top-export-control .primary");
  assert.match(await blockedExportButton.innerText(), /修复 1 条后导出/, "top export button should reflect hard blockers instead of promising an immediate export");
  assert.match(await blockedExportButton.locator("svg").getAttribute("class"), /lucide-circle-alert/, "blocked export action should use an alert icon, not the export arrow");
  assert.ok(await page.locator(".row-quality-badge").count() >= 1, "quality hints should appear on risky subtitle rows");
  await page.getByRole("button", { name: "跳到下一处质量提示" }).click();
  await page.waitForFunction(() => [...document.querySelectorAll(".current-segment-card textarea")].some((field) => field.value.includes("第三条也过短")));
  assert.match(await readWorkbenchFeedback(page), /已定位到质量提示/);
  await repairStatus.click();
  assert.match(await readWorkbenchFeedback(page), /时间轴问题/, "clicking the hard-blocker status should locate the issue before export");
  await page.waitForFunction(() => document.querySelector(".current-segment-card")?.innerText.includes("时间重叠"));
  const noTimingExportDownload = page.waitForEvent("download", { timeout: 700 }).then(() => false, () => true);
  await page.getByRole("button", { name: /修复 1 条后导出/ }).click();
  assert.equal(await noTimingExportDownload, true, "export should be blocked while timeline-overlap issues remain");
  assert.match(await readWorkbenchFeedback(page), /时间轴问题/, "timeline export block should explain the hard timing issue");
  await page.waitForFunction(() => document.querySelector(".current-segment-card")?.innerText.includes("时间重叠"));
  await page.getByRole("button", { name: "撤销", exact: true }).click();
  await page.waitForFunction(() => document.querySelectorAll(".review-list-row").length === 3);
  ...
  await page.waitForFunction(() => document.querySelector(".media-preview video") && document.querySelectorAll(".review-list-row").length === 3);
  assert.match(await readWorkbenchFeedback(page), /已关联[\s\S]*已保留 3 条转写段落/);
  ...
  await page.waitForTimeout(200);
  await page.getByRole("button", { name: "撤销", exact: true }).click();
  await page.waitForFunction(() => !document.querySelector(".media-preview video") && !document.querySelector(".media-panel") && document.querySelectorAll(".review-list-row").length === 3);
  await openMediaAssociation(page);
  ...
  assert.equal(await page.locator(".review-list-row").count(), 3, "importing over existing review rows should wait for explicit confirmation");
  ...
  assert.equal(await page.locator(".review-list-row").count(), 3, "canceling import replacement should keep existing rows");
  await chooseFile(page, page.getByRole("button", { name: "替换转写文件", exact: true }), sampleSubtitlePath);
  ...
  await page.getByRole("button", { name: "撤销", exact: true }).click();
  await page.waitForFunction(() => document.querySelector("[aria-label='当前段落说话人归属']")?.value === "Speaker 1");
  await page.waitForFunction(() => document.querySelector(".review-list-row")?.innerText.includes("00:04.500 - 00:07.500"));
  ...
  await page.getByRole("button", { name: "撤销", exact: true }).click();
  await page.waitForFunction(() => document.querySelector(".review-list-row")?.innerText.includes("00:04.500 - 00:08.000"));
  await page.getByRole("button", { name: "撤销", exact: true }).click();
  await page.waitForFunction(() => document.querySelector(".review-list-row")?.innerText.includes("00:05.000 - 00:08.000"));
  ...
  assert.match(insertedRows[3], /小数时间范围/);
  assert.match(await page.locator(".current-segment-card").innerText(), /3\/4/);
  await page.getByRole("button", { name: "撤销", exact: true }).click();
  await page.waitForFunction(() => document.querySelectorAll(".review-list-row").length === 3);
