...
![回响工作台首页](public/assets/echo-workbench-home.png)
...
需要验证真实工作台点击链路时，可以运行 UI 级 ASR 烟测。它会启动隔离本地服务，生成一段英文 WAV 样本，验证“上传音频 → 点击开始转写 → 生成校对表”，并确认不兼容的 NVIDIA Riva 非英文源语言会在前端阻断且不会发起 ASR 请求：

```bash
NVIDIA_API_KEY=你的NVIDIA密钥 npm run test:asr-ui-live
```
